...
- **Spec reconciliation must cover discovered limitations and behavioral findings, not just implementation details.** A chosen constant, a resolved ambiguity, or a schema/stack correction is not the whole bar. If real-world testing reveals something that changes what a spec-defined behavior can actually guarantee (e.g., a data source can't reliably signal "no data" the way a spec assumed), that finding must be written into `PRD_North_Star.md` and/or `Architecture_North_Star.md` as an explicit, named limitation — cross-referenced to the relevant section — not left implicit in a chat report or silently absorbed into an unrelated correction that doesn't mention the behavioral consequence. Before writing the commit message, explicitly re-scan the session's discoveries against this broader bar, not just "did I note the constants I chose."
