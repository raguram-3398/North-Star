"""Tests for patches/patch_manager.py: confidence branching and delivery ordering."""
...
def test_order_pending_items_orders_multiple_patches_by_hierarchy_position() -> None:
    """Input order (simulating arbitrary detection/arrival order) must not affect the result — only hierarchy_position does."""
...
def test_order_pending_items_mixes_a_patch_and_a_regular_topic() -> None:
    """A patch-note and a regular topic competing for the same day are ordered together purely by hierarchy_position."""
...
def test_order_pending_items_interleaves_multiple_patches_and_topics() -> None:
    """Two patches and two topics whose correct order requires interleaving by position, not grouping by kind."""
...
def test_decide_patch_delivery_orders_high_confidence_by_hierarchy() -> None:
    """Among two high-confidence patches listed out of hierarchy order, the earliest by position must be chosen."""
    patch_listed_first = {
...
def test_decide_patch_delivery_prioritizes_high_confidence_over_low() -> None:
    """A high-confidence patch is always chosen over a low-confidence one, regardless of hierarchy_position."""
