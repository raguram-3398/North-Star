...
- `.agent/skills/verification_question_generator/generator.py` imports `agents/research_outline_agent.py`'s Gemini call/timeout/error-handling helper directly (an underscore-prefixed, module-private-by-convention name) rather than that infrastructure living in a shared `src/utils/` module both could import as peers — a real architectural seam, not solved here since fixing it would mean refactoring already-tested, already-committed Agent code as a side effect of this Skill-scoped task. Flagged during implementation, not from original design.
