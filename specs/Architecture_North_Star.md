...
│   ├── cron/
│   │   └── refresh_roles.py            # refresh_roles_cache (shared, calls ground_role + upsert_role) + get_stale_or_missing_roles/check_and_refresh_stale_roles (startup staleness check) — called by GitHub Action (__main__ block) and startup check
│   ├── data/
