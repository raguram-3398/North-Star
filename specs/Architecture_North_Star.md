...
- ~~**Not yet built:** test-out (verification-first)~~ — **Resolved** (the test-out task, see the dedicated "Resolved" block below): `complete_topic_test_out`/`generate_gap_study_content`. ~~**Acting on the persisted pace signal**~~ — **Resolved** (the sustained-drift-wiring task, see the dedicated "Resolved" block below): `maybe_trigger_enrichment` (sustained-ahead) and `data/users.py`'s `extend_pacing` (sustained-behind). **Still not built** (deliberately deferred, not stubbed): patch-note delivery/surfacing into the day-content spillover mechanism, goal-completion closing-note content.
...
│   │   └── coaching_pace_agent.py      # reasoning/generation only: day-content generation, verification retry-cap, complete_topic_verification (pace-signal computation + drift wiring: maybe_trigger_enrichment on "ahead", extend_pacing on "behind"), test-out — see §3
│   ├── security/
