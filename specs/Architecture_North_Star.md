...
- **`is_goal_complete`/`generate_closing_note` are deliberately not wired to each other or to any trigger** — per this task's explicit scope, both are standalone, independently callable/testable functions; a future orchestration layer decides when to call either.
...
│   │   ├── users.py                    # get_user + extend_pacing (sustained-behind pacing-extension mechanism) — see §3/§5
