...
**Known limitation, discovered while mapping `PatchNote` to a real `Base` subclass (the significant-event-wiring task) — applies to every table above, not just `patch_notes`:** there is no migration tool and no schema-push mechanism anywhere in this codebase. `pyproject.toml` does not depend on Alembic (or any other migration tool); no code anywhere calls `Base.metadata.create_all()`; there is no `.sql` DDL file, no `Dockerfile`, and no `requirements.txt` (none of the three exist yet in this repo at all — separate, already-known gaps, not new). Every table in this section is, today, only a Python class definition in `models/schemas.py` — nothing in this codebase has ever executed DDL against a real Postgres/Neon instance (consistent with `db/connection.py` and every `data/` module's tests being exercised exclusively against a mocked `Session`, never a live database). **Before any live/demo run touches Neon, the tables above must be created by some other means** — the two lowest-effort options are a one-time `Base.metadata.create_all(get_engine())` call (e.g. added to `db/connection.py` or a small bootstrap script) or introducing Alembic; neither is built here. Flagged now because `patch_notes` becoming a real mapped model was the first task to make this gap concrete enough to name, not because this task introduced it — the same gap has silently applied to `roles_cache`/`outline_topics`/`progress_log`/`verification_attempts`/`pace_snapshots`, and now `users` (mapped by the sustained-drift-wiring task), since each was first mapped.
...
- **No migration tool or schema-push mechanism exists for any table in §5's Data Model — see §5's "Known limitation" for the full finding.** No Alembic, no `Base.metadata.create_all()` call anywhere, no DDL file. Every table (`roles_cache` through `patch_notes`) is, today, only a Python class definition — none has ever been created in a real Postgres/Neon instance. Discovered while mapping `PatchNote` to a real model (this task), not introduced by it.
